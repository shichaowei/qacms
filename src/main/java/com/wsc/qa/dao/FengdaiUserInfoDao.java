...
public class FengdaiUserInfoDao {
	...
	public void deleteAllLoanByLoginname(String loginname) {
		String sqltemp = ""
				...
				+ "DELETE FROM fengdai_channel.apply_to_third WHERE apply_id in (select id FROM fengdai_riskcontrol.loan_apply WHERE login_name = '%s');"
...
//		System.out.println(sql.split(";"));
	...
	public void deleteLoanByLoanName(String loanname) {
		String sqltemp = "DELETE FROM fengdai_riskcontrol.loan_apply WHERE loan_name LIKE '%s';"
				...
				+ "DELETE FROM`fengdai_shop`.`shop_order_after_sales`WHERE`loan_apply_id`NOT IN(SELECT id FROM fengdai_riskcontrol.loan_apply);DELETE FROM`fengdai_channel`.`apply_to_third`WHERE apply_id NOT IN(SELECT id FROM fengdai_riskcontrol.loan_apply);DELETE FROM`fengdai_riskcontrol`.`apply_astrict`;";
